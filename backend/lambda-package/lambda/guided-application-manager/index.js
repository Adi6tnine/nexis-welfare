...
async function handleStart(event) {
    ...
    await dynamoClient.send(new client_dynamodb_1.PutItemCommand({
        ...
        Item: (0, util_dynamodb_1.marshall)(application)
    }));
...
function generateApplicationSteps(schemeId) {
...
async function prefillFromProfile(userId, steps) {
    // In real implementation, fetch user profile from DynamoDB
    // For prototype, return empty object
    return {};
}
async function generateStepGuidance(step, formData) {
    const prompt = `
...
Provide a short, friendly guidance message (max 50 words) to help the user fill this field correctly.`;
    try {
        const guidance = await (0, bedrock_1.invokeClaudeModel)(prompt, undefined, 200);
...
async function getAISuggestion(step, userInput, errors) {
    const prompt = `
...
Provide a helpful suggestion in simple language (max 50 words) to help the user correct their input.`;
    try {
        return await (0, bedrock_1.invokeClaudeModel)(prompt, undefined, 200);
    }
    catch (error) {
        return 'Please check your input and try again.';
    }
}
function validateCompleteApplication(application) {
    const missingFields = [];
...
function generateApplicationSummary(application) {
    return {
...
function generateTrackingNumber(application) {
...
async function loadApplication(applicationId) {
    const response = await dynamoClient.send(new client_dynamodb_1.GetItemCommand({
        TableName: APPLICATIONS_TABLE,
        Key: (0, util_dynamodb_1.marshall)({ applicationId })
    }));
