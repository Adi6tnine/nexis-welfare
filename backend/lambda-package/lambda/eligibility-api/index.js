...
const dynamodb = new client_dynamodb_1.DynamoDBClient({ region: process.env.AWS_REGION || 'ap-south-1' });
...
async function handler(event) {
    try {
        ...
        for (const scheme of schemes) {
            const result = await (0, eligibility_v2_1.evaluateEligibilityV2)(profile, scheme);
            results.push(result);
        }
        ...
        for (const result of results.slice(0, 5)) {
            ...
            if (scheme) {
                const predictions = await (0, eligibility_v2_1.predictTimeline)(profile, scheme, result);
