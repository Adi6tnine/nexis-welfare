...
exports.extractRelevantSnippets = extractRelevantSnippets;
exports.identifySchemeEntities = identifySchemeEntities;
// @ts-nocheck
 ...
 */
function calculateRelevanceScore(document, keywords) {
 ...
 */
async function retrieveRelevantDocuments(query, userProfile, maxDocuments = 3) {
    try {
        ...
        // Fetch all schemes
        const schemes = await (0, s3_1.fetchAllSchemes)();
        if (schemes.length === 0) {
        ...
        for (const scheme of relevantSchemes.slice(0, 10)) { // Limit to 10 schemes
            ...
            documentPromises.push((0, s3_1.fetchPolicyDocument)(scheme.schemeId).then(content => {
                if (!content)
                    return null;
            ...
            // Fetch FAQ document
            documentPromises.push((0, s3_1.fetchFaqDocument)(scheme.schemeId).then(content => {
                if (!content)
                    return null;
                const score = calculateRelevanceScore(content, keywords);
                return {
        ...
        const scoredDocuments = documents
            .filter((doc) => doc !== null && doc.score > 0)
 ...
 */
function extractRelevantSnippets(document, keywords, snippetLength = 200) {
    const snippets = [];
    const docLower = document.toLowerCase();
    for (const keyword of keywords) {
 ...
 */
function identifySchemeEntities(query) {
    ...
    const schemeNames = schemePatterns.filter(pattern => queryLower.includes(pattern));
    // Benefit types
    const benefitPatterns = [
        'money', 'cash', 'financial', 'subsidy', 'pension',
        'insurance', 'health', 'education', 'housing', 'loan'
    ];
    const benefitTypes = benefitPatterns.filter(pattern => queryLower.includes(pattern));
    // Document types
    const documentPatterns = [
        'aadhar', 'aadhaar', 'pan', 'ration card', 'income certificate',
        'caste certificate', 'disability certificate', 'bank account'
    ];
    const documentTypes = documentPatterns.filter(pattern => queryLower.includes(pattern));
    return {
        schemeNames,
        benefitTypes,
        documentTypes
