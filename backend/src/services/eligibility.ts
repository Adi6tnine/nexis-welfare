import { UserProfile, Scheme, EligibilityCriteria, CriterionResult } from '../models';
...
export function evaluateEligibility(
  profile: UserProfile,
  scheme: Scheme
): { isEligible: boolean; results: CriterionResult[]; matchScore: number } {
  ...
  if (incomeResult) results.push(incomeResult);
  ...
  if (occupationResult) results.push(occupationResult);

  const genderResult = evaluateGenderCriterion(profile, criteria);
  if (genderResult) results.push(genderResult);

  const socialCategoryResult = evaluateSocialCategoryCriterion(profile, criteria);
  if (socialCategoryResult) results.push(socialCategoryResult);
 ...
 */
export function evaluateAgeCriterion(
  profile: UserProfile,
 ...
 */
export function evaluateIncomeCriterion(
  profile: UserProfile,
  criteria: EligibilityCriteria
): CriterionResult | null {
 ...
 */
export function evaluateOccupationCriterion(
  profile: UserProfile,
  criteria: EligibilityCriteria
): CriterionResult | null {
 ...
 */
export function evaluateGenderCriterion(
  profile: UserProfile,
  criteria: EligibilityCriteria
): CriterionResult | null {
 ...
 */
export function evaluateSocialCategoryCriterion(
  profile: UserProfile,
  criteria: EligibilityCriteria
): CriterionResult | null {
 ...
 */
export function evaluateDisabilityCriterion(
  profile: UserProfile,
  criteria: EligibilityCriteria
): CriterionResult | null {
 ...
 */
export function getSatisfiedCriteria(results: CriterionResult[]): string[] {
  return results
 ...
 */
export function getUnsatisfiedCriteria(results: CriterionResult[]): Array<{ criterion: string; reason: string }> {
  return results
    .filter(r => !r.satisfied)
    .map(r => ({
