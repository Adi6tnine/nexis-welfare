...
Example: {"age": 45, "occupation": "Farmer"}`;
  ...
  try {
    const response = await invokeClaudeModel(prompt, undefined, 500);
...
function determineNextQuestion(context: ConversationContext, language: string): string {
...
function getQuestions(language: string): Record<string, string> {
...
function calculateProgress(context: ConversationContext): number {
  const totalFields = 7; // Basic fields to collect
  return Math.round((context.completedFields.length / totalFields) * 100);
}

function isProfileComplete(context: ConversationContext): boolean {
  const requiredFields = ['age', 'occupation', 'state', 'annualIncome'];
  return requiredFields.every(field => 
    context.completedFields.includes(field) || context.collectedData[field]
  );
}
...
async function loadContext(sessionId: string): Promise<ConversationContext | null> {
  try {
    const response = await dynamoClient.send(new GetItemCommand({
      ...
      Key: marshall({ sessionId, sk: 'CONTEXT' })
    }));
    
    if (response.Item) {
      return unmarshall(response.Item) as ConversationContext;
    }
    return null;
  } catch (error) {
