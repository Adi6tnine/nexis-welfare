...
async function checkNewSchemes(body: any) {
  const { userId } = body;
  ...
  try {
    ...
    const schemesResult = await docClient.send(new ScanCommand({
    ...
    for (const scheme of newSchemes) {
      const eligibility = await evaluateEligibilityV2(userProfile, scheme);
...
async function checkEligibilityChanges(body: any) {
  const { userId } = body;
...
async function getUserAlerts(body: any) {
...
async function markAlertRead(body: any) {
...
async function dismissAlert(body: any) {
  const { alertId } = body;
...
function createMockNewSchemesResponse(userId: string) {
...
function createMockEligibilityChangesResponse(userId: string) {
  const mockAlerts: Alert[] = [
    {
...
function createMockUserAlertsResponse(userId: string, unreadOnly: boolean) {
